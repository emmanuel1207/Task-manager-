...
def reg_user(): 
      ...
      # Exception handling for if user inputs the same username twice
      ...
      if new_password == confirm_password:
            ...
            with open("user.txt", "w") as out_file:
                ...
                out_file.write("\n".join(user_data))
      # Exception handling for if new password does not match confirmed password
      else:
...
def view_mine():
        ...
        # If statement - '-1' input from user takes them back to the menu
        if task_selection == int(-1):
            menu
        else:
            # 'user_selected_option' variable - takes the user input and subtracts 1 so that the value can be used to iterate over the task data 
            ...
            if user_selected_option >= len(task_selection_list_for_operation):
             ...
             print("\n")
             # 'edit_or_complete_input' variable - asks user how they would like to modify the task, with exceptions for tasks that have been completed
...
def generate_reports():
        ...
        if not os.path.exists("User overview.txt"):
         with open("User overview.txt", "w") as user_overview_file:
          pass
        # Reading in 'tasks.txt' and 'user.txt' data and generating statistics
        with open("user.txt", 'r') as user_file:
         user_data = user_file.read().split("\n")
        ...
        # Reading in task data
        with open("tasks.txt", 'r') as task_file:
          task_data = task_file.read().split("\n")
          joined_tasks = ";".join(task_data)
          full_task_list = joined_tasks.split(";")
          full_completed_tasks_dict = {}
          # Exception handling for when there are no tasks
          for e in task_list_for_comparing_dates:
              if len(e)<= 1:
                  print("There are no complete tasks.")
              else:
               # Creating dictionary value for completed tasks              
        ...
        with open("tasks.txt", 'r') as task_file:
          ...
          for e in task_list_for_comparing_dates:
              if len(e)<= 1:
               ...
               # Creating dictionary value for incomplete tasks      
               for f in full_user_list[::2]:
        ...
        with open("tasks.txt", 'r') as task_file:
          ...
          for e in task_list_for_comparing_dates:
              if len(e)<= 1:
               ...
               # Creating dictionary value for overdue tasks       
               for f in full_user_list[::2]:
                overdue_tasks_dict[f] = 0
                for e in task_list_for_comparing_dates:
                 if e[5] == 'No' and e[3] <= e[4] and e[0] == f:
                    overdue_tasks_dict[f] += 1
        # User overview report generated, with exception handling for when there are no tasks
        for e in task_list_for_comparing_dates:
            if len(e)<= 1:
                print("There are no tasks for the report.")
            else:
             # Writing statistics into user overview report, with exception handling for when there are no tasks
             with open("User overview.txt", "w") as user_overview_file:
              ...
              for f in full_user_list[::2]:
               ...
               user_overview_file.write("\n")
              for f in full_user_list[::2]:
                 if full_completed_tasks_dict[f] == 0:
                  ...
                  user_overview_file.write("\n")
              for f in full_user_list[::2]:
                 if full_incomplete_tasks_dict[f] == 0:
                    print("There are no incomplete tasks.")
                 else:
        ...
        # Task overview report, with exception handling for when there are no tasks
        ...
        for e in task_list_for_comparing_dates:
           if len(e)<= 1:
            ...
            # Writing statistics into task overview report, with exception handling for when there are no tasks
            with open("Task overview.txt", "w") as task_overview_file:
             task_no = (f"Total Tasks: {total_task_count}")
...
# While loop - continuously presents menu, with exception handling for incorrect username or password
